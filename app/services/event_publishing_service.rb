...
class EventPublishingService
  ...
  def call
    ...
    return { success: false, error: 'Event must have schedule' } unless @event.event_schedule.any?
  ...
  def validate_event
    @event.event_skill_requirements.any? && @event.event_schedule.any?
  end
  ...
  def generate_shifts_for_skill_requirement(skill_requirement)
    skill_requirement.needed_workers.times do
      @event.shifts.create!(
        ...
        capacity: 1,
        start_time_utc: @event.event_schedule.first.start_time_utc,
        end_time_utc: @event.event_schedule.first.end_time_utc,
        status: 'published',
