...
**Security Note:** ⚠️ **NEVER commit actual secret values to git.** This document lists variable names and purposes only.
...
- **Security:** ⚠️ **CRITICAL SECRET** - Never expose in logs or git
...
- **Security:** ⚠️ **CRITICAL SECRET** - Never expose
...
- **Security:** ⚠️ **API KEY** - Restrict to specific domains/IPs in Google Cloud Console
...
- **Security:** ⚠️ **SENSITIVE** - Don't expose publicly
...
- **Security:** ⚠️ **API KEY** - Restrict in Google Cloud Console
...
**⚠️ CRITICAL:** Production secrets must be different from staging.
...
**⚠️ WARNING:** Rotating `SECRET_KEY_BASE` will invalidate all existing sessions. Users will need to log in again.
...
|----------|----------|-------------|----------|----------|
| `RAILS_ENV` | ✅ | All | No | Heroku |
| `RAILS_MASTER_KEY` | ✅ | Prod/Staging | ⚠️ Critical | No |
| `SECRET_KEY_BASE` | ✅ | Prod/Staging | ⚠️ Critical | No |
| `DATABASE_URL` | ✅ | All | ⚠️ Sensitive | Heroku |
| `RAILS_LOG_TO_STDOUT` | ✅ | Heroku | No | No |
| `RAILS_SERVE_STATIC_FILES` | ✅ | Heroku | No | No |
| `NODE_ENV` | ✅ | All | No | Heroku |
| `VITE_API_URL` | ✅ | Prod/Staging | No | No |
| `SMTP_HOST` | ❌ | Optional | No | No |
| `SMTP_USERNAME` | ❌ | Optional | ⚠️ Secret | No |
| `SMTP_PASSWORD` | ❌ | Optional | ⚠️ Critical | No |
| `GOOGLE_PLACES_API_KEY` | ❌ | Optional | ⚠️ API Key | No |
| `SENTRY_DSN` | ❌ | Optional | ⚠️ Sensitive | No |
