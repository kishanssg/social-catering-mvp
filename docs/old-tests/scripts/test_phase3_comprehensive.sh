...
# 📋 PHASE 3: WORKERS PAGE AUDIT - COMPREHENSIVE TEST SUITE
# Tests all critical functionality for Workers management

set -e  # Exit on any error

BASE_URL="http://localhost:3000/api/v1"
COOKIES_FILE="test_cookies.txt"
TEST_RESULTS="phase3_test_results.txt"

echo "🧪 PHASE 3: WORKERS PAGE AUDIT - COMPREHENSIVE TEST SUITE" > $TEST_RESULTS
echo "=========================================================" >> $TEST_RESULTS
...
echo -e "${YELLOW}🚀 Starting Phase 3 Comprehensive Test Suite${NC}"
echo ""
...
# 2. WORKERS API TESTS
# ===========================================
echo -e "${YELLOW}📋 2. WORKERS API TESTS${NC}"
...
# Test workers count
run_test "Workers list returns correct count (25+)" \
    "check_json_response '$BASE_URL/workers' '.data.workers | length' '25'"
...
# Test skills count
run_test "Skills list returns correct count (14+)" \
    "curl -s '$BASE_URL/skills' | jq -r '.data | length' | grep -q '1[4-9]'"
...
# ===========================================
# 5. CERTIFICATIONS API TESTS
# ===========================================
echo -e "${YELLOW}📋 5. CERTIFICATIONS API TESTS${NC}"

# Test certifications list
run_test "Certifications list endpoint works" \
    "check_response_contains '$BASE_URL/certifications' '.data'"

# Test certifications count
run_test "Certifications list returns correct count (4+)" \
    "check_json_response '$BASE_URL/certifications' '.data | length' '4'"

# Test add certification to worker
run_test "Add certification to worker" \
    "curl -X POST '$BASE_URL/workers/$CREATED_WORKER_ID/certifications' -H 'Content-Type: application/json' -b '$COOKIES_FILE' -d '{\"certification_id\":67}' -s | jq -r '.status' | grep -q 'success'"

# Test duplicate certification detection
run_test "Duplicate certification detection works" \
    "curl -X POST '$BASE_URL/workers/$CREATED_WORKER_ID/certifications' -H 'Content-Type: application/json' -b '$COOKIES_FILE' -d '{\"certification_id\":67}' -s | jq -r '.status' | grep -q 'error'"
...
# ===========================================
# 7. BULK ASSIGNMENT TESTS (CRITICAL)
# ===========================================
echo -e "${YELLOW}📋 7. BULK ASSIGNMENT TESTS (CRITICAL)${NC}"
...
# Test bulk assignment API endpoint
run_test "Bulk assignment API endpoint exists" \
...
# Test invalid shift ID in bulk assignment
run_test "Invalid shift ID in bulk assignment returns error" \
...
echo "Success Rate: $(( TESTS_PASSED * 100 / TOTAL_TESTS ))%" >> $TEST_RESULTS
...
if [ $TESTS_FAILED -eq 0 ]; then
    ...
    echo "🎉 ALL TESTS PASSED! Phase 3 is working perfectly!" >> $TEST_RESULTS
    exit 0
else
    echo -e "${RED}⚠️  Some tests failed. Check the results above.${NC}"
    echo "⚠️  Some tests failed. Check the results above." >> $TEST_RESULTS
    exit 1
fi
